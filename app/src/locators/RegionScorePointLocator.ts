...
export class RegionScorePointLocator extends Locator {
  ...
  getPositionOnParent(location: Location, context: MaterialContext) {
    const region = context.rules.material(MaterialType.Region).getItem<Region>(location.parent!).id
...
export const RegionScoreY: Partial<Record<Region, number>> = {
  ...
  [Region.RedSky21]: 45,
  [Region.GreenSky9]: 30,
  [Region.GreenSky33]: 45,
  [Region.YellowSky4]: 44,
  [Region.YellowSky20]: 30,
  [Region.YellowSky38]: 44,
  [Region.GraySky7]: 46,
  [Region.GraySky13]: 46,
  [Region.GraySky29]: 48
}
